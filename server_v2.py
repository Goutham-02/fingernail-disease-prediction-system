...
app = FastAPI(title="Nail Disease Diagnosis API V2 (with LLM Refinement)", version="2.0.0")
...
def llm_refine_prediction(top_3_candidates: list, symptoms: str, disease_db: list) -> dict:
    """
    Use LLM to refine prediction based on top 3 CNN candidates and symptoms.
    
    Args:
        top_3_candidates: Top 3 predictions with scores
        symptoms: User reported symptoms
        disease_db: Disease database for context
    
    Returns:
        Dict with final_diagnosis, confidence, reasoning
    """
    try:
        if not os.getenv("GEMINI_API_KEY"):
            logger.warning("Gemini API key not configured for refinement")
            return None
        
        if not symptoms or not symptoms.strip():
            logger.info("No symptoms provided, skipping LLM refinement")
            return None
        
        # Build context for each candidate
        candidates_info = []
        for i, candidate in enumerate(top_3_candidates[:3], 1):
            disease_name = candidate.get("disease", "")
            
            # Find disease info
            disease_ctx = {}
            for d in disease_db:
                if str(d.get("name", "")).lower().strip() == disease_name.lower().strip():
                    disease_ctx = d
                    break
            
            candidate_text = f"""
{i}. {disease_name}
   - Combined Score: {candidate.get('aggregated_score', 0)*100:.1f}%
   - Model Probability: {candidate.get('avg_model_probability', 0)*100:.1f}%
   - Visual Evidence: {candidate.get('avg_visual_score', 0)*100:.1f}%
   - Symptom Match: {candidate.get('avg_symptom_score', 0)*100:.1f}%
   
   Medical Info:
   - Category: {disease_ctx.get('category', 'Unknown')}
   - Common Symptoms: {', '.join(disease_ctx.get('associated_symptoms', [])[:5])}
   - Visual Features: {', '.join(disease_ctx.get('visual_features', [])[:5])}
"""
            candidates_info.append(candidate_text)
        
        # Construct prompt
        prompt = f"""You are a medical AI assistant specializing in nail disease diagnostics.

TASK: Analyze the CNN model's top 3 predictions alongside user-reported symptoms to determine the most likely diagnosis.

CNN MODEL PREDICTIONS (Top 3):
...
INSTRUCTIONS:
1. Carefully compare each condition's expected symptoms against what the user reported
2. Consider the CNN's visual evidence strength (how confident the model is based on the image)
3. Weigh clinical likelihood - do the symptoms align better with a different condition than the top CNN prediction?
4. If symptoms strongly contradict the top prediction, consider re-ranking

Output your analysis in this EXACT JSON format (valid JSON only, no markdown):
{{
  "final_diagnosis": "disease_name",
  "confidence": 0.85,
  "reasoning": "Brief 2-3 sentence explanation of why this diagnosis is most likely given the symptoms and visual evidence",
  "differential_concerns": ["condition2", "condition3"]
}}
...
CRITICAL: Return ONLY the JSON object, no additional text or markdown formatting."""
        ...
        result_text = _extract_gemini_text(response)
        if not result_text:
            logger.warning("Empty response from Gemini refinement")
            return None
        ...
        logger.info(f"LLM refined prediction: {result.get('final_diagnosis')} (confidence: {result.get('confidence')})")
        return result
...
async def diagnose(
    ...
    try:
        ...
        # Extract aggregated prediction data
        aggregated = results.get("aggregated_prediction", {})
        top_3_candidates = aggregated.get("top_3_candidates", [])
        cnn_prediction = aggregated.get("predicted_disease", "")
        cnn_score = aggregated.get("aggregated_score", 0.0)
        
        # Collect Grad-CAM URLs
        ...
        if symptoms and symptoms.strip() and len(top_3_candidates) >= 3:
            ...
            if llm_refined:
                final_prediction = llm_refined.get("final_diagnosis", cnn_prediction)
                final_score = llm_refined.get("confidence", cnn_score)
                final_reasoning = llm_refined.get("reasoning", "")
                logger.info(f"LLM refined: {cnn_prediction} ({cnn_score:.2f}) → {final_prediction} ({final_score:.2f})")

        # Lookup disease info for final prediction
        info = None
        for d in DISEASE_DB:
            if str(d.get("name", "")).lower() == str(final_prediction).lower():
                info = d
                break
        ...
        final_payload = {
            ...
            "llm_refined_prediction": {
                ...
                "was_refined": llm_refined is not None,
                "differential_concerns": llm_refined.get("differential_concerns", []) if llm_refined else []
            } if symptoms and symptoms.strip() else None,
